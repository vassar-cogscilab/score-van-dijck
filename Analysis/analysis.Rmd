...
rt.inclusion <- all.parity.data %>%
  ...
  filter(correct==TRUE) %>%
  summarize(median = median(rt)) %>%
  mutate(rt.include = median < (group.rt$group.median + 4*group.rt$group.sd) && median > 200)
...
```{r message=FALSE, warning=FALSE}
anova.model <- ezANOVA(lm.result, dv = dRT.coefficient, between = wmtask, within = phase, within_covariates = phase.diff, wid = subject)
